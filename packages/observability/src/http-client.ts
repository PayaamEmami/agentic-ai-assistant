import { fetchWithTelemetry } from './http.js';
...
export function createHttpClient(options: { component: string; eventPrefix: string }) {
  async function performRequest(input: string | URL | Request, init?: RequestInit): Promise<Response> {
    const url = new URL(toUrlString(input));
    const response = await fetchWithTelemetry(url.toString(), init, {
  ...
  return {
    async requestJson<T>(input: string | URL | Request, init?: RequestInit): Promise<T> {
      const response = await performRequest(input, init);
      return response.json() as Promise<T>;
    },
    async requestText(input: string | URL | Request, init?: RequestInit): Promise<string> {
      const response = await performRequest(input, init);
